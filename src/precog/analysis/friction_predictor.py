...
class FrictionPredictor:
    def __init__(self, model_path='friction_model.joblib', config=None):
        self.model_path = model_path
        self.model = Pipeline([
            ('scaler', StandardScaler()),
            ('rf', RandomForestClassifier(random_state=random_state, class_weight='balanced'))
    ...
    def predict_friction_risk(self, current_data_per_location: pd.DataFrame) -> list[Any] | None:
        ...
        for i, prob in enumerate(probabilities):
            location_name = current_data_per_location.index[i] if current_data_per_location.index.name else f"Location_{i}"
            # Simplified timeline and factors for now
            timeline = "medium-term"
            if prob > 0.7: timeline = "short-term"
            if prob > 0.9: timeline = "imminent"

            # Identify top contributing factors (simplified - based on feature values)
            # A more robust way would use SHAP values or feature importance from the model
            location_features = features.iloc[i]
            # Get names of features that have non-zero values for this location
            active_factors = [fname for fname in self.feature_names if location_features[fname] != 0]

            risks.append(FrictionRisk(
                location=location_name,
                risk_score=float(prob),
                predicted_timeline=timeline,
                risk_factors=active_factors[:3], # Top 3 active factors
                confidence=float(self.model.predict(features[i:i+1])[0] == (prob > 0.5)) # Simplistic confidence
            ))
