...
class HyperlocalIntelligenceSystem:
    ...
    def _update_system_metrics(self, analysis_results: Dict[str, Any]):
        ...
        risk_levels = [risk.risk_score for risk in analysis_results.get('friction_risks', []) if hasattr(risk, 'risk_score')]
    ...
    def generate_report(self, analysis_results: Dict[str, Any]) -> str:
        ...
        if risks:
            high_risks_count = len([r for r in risks if hasattr(r,'risk_score') and r.risk_score > 0.6])
            report.append(f"Total Risk Areas: {len(risks)}, High Risk Areas: {high_risks_count}")
            for risk in sorted(risks, key=lambda x: getattr(x, 'risk_score', 0), reverse=True)[:3]:
                factors = ", ".join(getattr(risk,'risk_factors',[]))
                report.append(f"  ⚠️ {getattr(risk,'location','N/A')} - Score: {getattr(risk,'risk_score',0):.2f}, Timeline: {getattr(risk,'predicted_timeline','N/A')}, Factors: {factors}")
            report.append("")
    ...
    def run_continuous_monitoring(self, interval_minutes: int = 30, run_once: bool = False):
        ...
        while True:
            try:
                ...
                results = self.analyze_current_situation(data)
